...
export const get_exception_string = data => {
  ...
  if (is_app) {
    ...
    return return_string ? `<div>${return_string}</div>` : `<div>${data.message}</div>`;
  }
};
...
export const get_currency_string = (amount, currency_type = '') => {
  let string = '';
  let currency =
    Config.CURRENCY_INFO && Config.CURRENCY_INFO[currency_type] ? Config.CURRENCY_INFO[currency_type] : Config.CURRENCY;
...
const api_res = {
  'user.payment.method_added': 'payout mode added successfully , verify from your email',
};
