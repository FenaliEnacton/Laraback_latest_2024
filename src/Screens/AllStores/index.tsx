...
const AllStores = props => {
  ...
  const [alphaStoresList, setAlphaStoresList] = useState([]);

  useEffect(() => {
    props.request_stores_alpha_char_list();
  ...
  useEffect(() => {
    if (props.alpha_stores) {
      setAlphaStoresList(props.alpha_stores);
    }
  }, [props.alpha_stores]);
  ...
  const render_stores = ({ item, index }) => {
    return (
      <TouchableOpacity
        style={[
          styles.alpha_btn,
          props.stores_alpha_char === item ? { zIndex: 99999 } : {},
        ]}
        onPress={() => props.request_stores_by_alpha(item)}>
        <Text
          style={[
            styles.sectionTitle,
            props.stores_alpha_char === item
  ...
  const render_store_by_alpha = ({ item, index }) => {
    return <TopStoreCard store={item} style={styles.storeCard} />;
  };

  const getHighestCashbackStore = () => {
    let percent = [];
    let fixed = [];
    percent = props.alpha_stores
      .filter(a => a.amount_type === 'percent')
      .sort((a, b) => Number(b.cashback_amount) - Number(a.cashback_amount));

    fixed = props.alpha_stores
      .filter(a => a.amount_type === 'fixed')
      .sort((a, b) => Number(b.cashback_amount) - Number(a.cashback_amount));

    setFilterToggle(!filterToggle);
    setAlphaStoresList(fixed.concat(percent));
  };

  const getMostPopularStore = () => {
    setFilterToggle(!filterToggle);
    setAlphaStoresList(props.alpha_stores.sort((a, b) => a.clicks - b.clicks));
  };
  ...
  return (
    <Container>
                ...
                <Menu
                  ...
                  <MenuOptions>
                    ...
                    <MenuOption
                      ...
                      <View style={styles.filterTextCard}>
                        <Text style={styles.filterText}>Highest Cashback</Text>
                      </View>
                    </MenuOption>
                  </MenuOptions>
                </Menu>
          ...
          {alpha_stores_loading ? (
            ...
            <>
              <ComponentAnimation index={4}>
                <FlatList
                  data={alphaStoresList}
                  // horizontal={true}
                  numColumns={2}
                  showsHorizontalScrollIndicator={false}
                  keyExtractor={(item, index) => index.toString()}
                  extraData={props}
                  renderItem={render_store_by_alpha}
                  style={styles.list}
      ...
      <Loader show={props.store_details_loading} />
    </Container>
  );
};
