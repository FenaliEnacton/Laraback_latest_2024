...
const useUserAccountSettings = () => {
  const [loadingUserProfileDetails, setLoadingUserProfileDetails] = useRecoilState(
  ...
  const [loadingUserChangePassword, setLoadingUserChangePassword] = useRecoilState(
  ...
  async function request_user_profile_details() {
    try {
      ...
      if (response.ok && response.data.success && !response.data.error) {
        ...
        response.data?.msg ? get_api_error_string(response.data?.msg) : response.problem;
  ...
  async function request_user_change_password(old_password, new_password, repeat_password, verify_token) {
    try {
      setLoadingUserChangePassword(true);
      const response = await user_api.user_auth_api(is_app() ? 'user.change.password' : 'user.change.web.password', {
      ...
      if (response.ok && response.data.success && !response.data.error) {
        if (is_app()) {
          Toast.successBottom(get_translation('user_dashboard.account_setting.password_change_success'));
        } else {
          show_success_message({
            text: get_translation('user_dashboard.account_setting.password_change_success'),
          });
        }
        ...
        if (is_app()) {
          Toast.errorBottom(
            response.data.data?.error
              ...
              : get_translation('user_dashboard.account_setting.change_password_error'),
          );
        } else {
          show_fail_message(
            response.data.data?.error
      ...
      show_fail_message({
        text: get_translation('user_dashboard.account_setting.change_password_error'),
      });
      setLoadingUserChangePassword(false);
      console.error(error);
      return false;
  ...
  async function request_update_profile_info(
    ...
    try {
      ...
      const response = await user_api.user_dashboard_post_api('user.profile.meta', {
        user_id: user_id,
        bio: bio,
        dob: dob,
      ...
      const name_response = await user_api.user_dashboard_post_api('user.profile.update.name', {
        name: user_name,
      ...
      if (
        ...
        if (is_app()) {
          Toast.successBottom(get_translation('user_dashboard.account_setting.profile_info_updated'));
        } else {
          show_success_message({
            text: get_translation('user_dashboard.account_setting.profile_info_updated'),
          });
        }
        ...
        if (is_app()) {
          Toast.errorBottom(get_translation('user_dashboard.account_setting.request_failed_try_again'));
        } else {
          show_fail_message({
            text: response.data.data?.error
              ? response.data.data.message
              : get_translation('user_dashboard.account_setting.request_failed_try_again'),
          });
        }
        response.data?.msg ? get_api_error_string(response.data?.msg) : response.problem;
        setLoadingUpdateUserInfo(false);
      ...
      if (is_app()) {
        Toast.successBottom(get_translation('user_dashboard.account_setting.request_failed_try_again'));
      } else {
        show_fail_message({
          text: get_translation('user_dashboard.account_setting.request_failed_try_again'),
        });
      }
      handle_api_error(error);
      setLoadingUpdateUserInfo(false);
